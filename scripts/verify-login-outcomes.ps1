...
Assert-Regex $handler '!hasClientVersion \|\| !requiredVersion\.Equals\(clientVersion\).*?Reject\(LoginFailType\.OldClient' "unsupported client mapping"
Assert-Regex $handler 'PenaltyLogDAO\.LoadByIp\(ipAddress\)\.Any\(\).*?Reject\(LoginFailType\.CantConnect' "IP penalty mapping"
