...
$booleanStateFields = @($stateFields | Where-Object { $_ -ne "authority" })
$expectedFields = @(
...
function Assert-ExactProperties {
    ...
    if ($unexpected.Count -gt 0 -or $missing.Count -gt 0) {
        throw "$Description properties do not match the contract. Unexpected=[$($unexpected -join ', ')] Missing=[$($missing -join ', ')]."
    }
}
...
function Assert-StateTypes {
    ...
    if ((Get-PropertyValue $State "authority") -notin @("user", "gm", "banned")) {
        throw "$Description authority is invalid."
    }
}
...
function Get-Section {
    ...
    if ($start -lt 0) {
        throw "Login source contract failed: $Description start marker was not found."
    }
    $end = $Content.IndexOf($EndMarker, $start + $StartMarker.Length, [StringComparison]::Ordinal)
    if ($end -lt 0) {
        throw "Login source contract failed: $Description end marker was not found."
    }
...
foreach ($fixtureCase in @($fixture.cases)) {
    ...
    if ($fixtureCase.id -notmatch '^[a-z][a-z0-9_]{2,63}$' -or $seen.ContainsKey($fixtureCase.id)) {
        throw "Invalid or duplicate fixture ID: $($fixtureCase.id)."
    }
...
Assert-Ordered $legacy @(
    ...
    "bool accountNameMatches = string.Equals(",
    "if (!ValidateAccountStateBeforeCredentials(loadedAccount))",
    "if (!PasswordHashService.VerifyLoginPayload(",
    "if (!ValidateClientVersion(hasClientVersion, clientVersion))",
    "await CompleteLoginAsync("
) "Legacy Login decision order must remain deterministic"

Assert-Regex $legacy 'LoadAccountByLoginName\(protocolUsername, resolvedRegionType\).*?Reject\(LoginFailType\.AccountOrPasswordWrong, "Session removed\. Reason: Unknown account"\)' "unknown account mapping"
Assert-Regex $legacy '!accountNameMatches.*?Reject\(LoginFailType\.WrongCaps' "wrong account casing mapping"
Assert-Regex $legacy '!PasswordHashService\.VerifyLoginPayload\(.*?Reject\(LoginFailType\.AccountOrPasswordWrong, "Session removed\. Reason: Wrong credentials"\)' "wrong credential mapping"
Assert-Regex $version '!TryParseVersion\(ServerConfiguration\.GameVersion, out Version requiredVersion\).*?Reject\(\s*LoginFailType\.CantConnect' "invalid server-version mapping"
Assert-Regex $version '!hasClientVersion \|\| !requiredVersion\.Equals\(clientVersion\).*?Reject\(LoginFailType\.OldClient' "unsupported client mapping"

Assert-Ordered $completion @(
    "PenaltyLogDAO.LoadByIp(ipAddress).Any()",
    "CheckIsConnectedAsync(loadedAccount.AccountId)",
    "IsAccountConnected(loadedAccount.AccountId)",
    "penalty != null || loadedAccount.Authority == AuthorityType.Banned",
...
) "Shared Login completion order must remain deterministic"
...
Assert-Regex $completion '!SynchronizeAccountLanguage\(loadedAccount, culture\).*?Reject\(LoginFailType\.CantConnect' "language synchronization failure mapping"
...
Assert-Contains $handler "Client has been removed. Reason: World Server not found" "missing World mapping"
...
Write-Host "Verified $($seen.Count) sanitized login outcome fixtures and consolidated production contracts."
