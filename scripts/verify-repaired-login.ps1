...
Require $login 'IsAccountSessionRegistered(loadedAccount.AccountId, newSessionId)' 'Modern Login entries no longer recognize their existing account/session tuple.'
Require $login '!gameforgeTicket.IsFirstConsumption' 'Login no longer distinguishes the initial entry from active-session continuations.'
Require $login 'Gameforge session is no longer active' 'A stale authorization code could recreate a disconnected session.'
...
Forbid $login 'MaximumConsumptionsPerTicket' 'Login still enforces the obsolete three-entry limit.'
Require $login 'accountRegistered && ownsAccountRegistration' 'Later modern Login entries could roll back a session they do not own.'
...
Require $store 'public static readonly TimeSpan MaximumActiveSessionLifetime = TimeSpan.FromHours(24);' 'Modern Login tickets must become bounded active-session leases after first use.'
Require $store 'public int ConsumptionCount' 'Active-session entries are not counted.'
Require $store 'if (ticket.SessionId <= 0)' 'The first modern entry no longer owns SessionId binding.'
Require $store 'ticket.SessionId = proposedSessionId;' 'The first modern entry no longer binds the stable SessionId.'
Require $store 'ticket.ExpiresAtUtc = nowUtc.Add(MaximumActiveSessionLifetime);' 'The short authorization ticket is not converted into a bounded active-session lease.'
Require $store 'SessionId = ticket.SessionId' 'Later modern entries no longer receive the stable SessionId.'
Require $store 'ConsumptionNumber = ticket.ConsumptionCount' 'Ticket entry ownership is no longer returned to Login.'
Require $store 'ticket.ConsumptionCount++;' 'Ticket consumption no longer advances the active-session entry counter.'
Forbid $store 'MaximumConsumptionsPerTicket' 'The obsolete three-consumption limit returned.'
Forbid $store 'RemainingConsumptions' 'The obsolete remaining-consumption counter returned.'
...
Write-Host 'Repaired Login -> Master -> World, renewable active-session tickets, ticket-bound modern region and ten-language contracts verified.'
