...
using Frostvein.Configuration.GameEvent;
...
namespace Frostvein.GameObject.Plugin.Event
{
    ...
    {
        public static EventType EventType => EventType.INSTANTBATTLE;
        ...
        {
            SendMessageWithDelay("Instant Battle will start in 5 Minute(s)", TimeSpan.FromMinutes(5));
            SendMessageWithDelay("Instant Battle will start in 1 Minute(s)", TimeSpan.FromMinutes(1));
            SendMessageWithDelay("Instant Battle will start in 30 Seconds", TimeSpan.FromSeconds(30));
            SendMessageWithDelay("Instant Battle will start in 10 Seconds", TimeSpan.FromSeconds(20));
            SendMessageWithDelay("Instant Battle has begun", TimeSpan.FromSeconds(10));
            ServerManager.Instance.Sessions.Where(s => s.Character?.MapInstance.MapInstanceType == MapInstanceType.BaseMapInstance).ToList().ForEach(s => s.SendPacket($"qnaml 1 #guri^506 Do you want to join the Battle?"));
            ServerManager.Instance.EventInWaiting = true;
            SendMessageWithDelay("Instant Battle started", TimeSpan.FromSeconds(30));
            ...
            {
                var toAddSessions = sessions.Where(s => s.Character.Level >= kvp.Key && s.Character.Level <= kvp.Value && !s.Character.IsMuted()).ToList();
        ...
        public class InstantBattleTask
        {
            ...
            public static void HandleRewards(ClientSession cli)
            {
