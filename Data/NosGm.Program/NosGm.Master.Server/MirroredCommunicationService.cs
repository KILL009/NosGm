...
namespace NosGm.Master.Server
{
          ...
          ICommunicationService
    {
        ...
        public new void UpdateRelation(string worldGroup, long relationId)
        {
            ...
            MasterRelationRefreshGrpcAuthority.Instance.Publish(
                worldGroup,
                relationId);
        }
    ...
    internal sealed class MasterRelationRefreshGrpcAuthority
    {
        private static readonly Lazy<MasterRelationRefreshGrpcAuthority>
            LazyInstance =
                new Lazy<MasterRelationRefreshGrpcAuthority>(
                    () => new MasterRelationRefreshGrpcAuthority());

        private const int MaximumAttempts = 3;
        private const int InitialRetryDelayMilliseconds = 100;

        private MasterRelationRefreshGrpcAuthority()
        {
        }

        public static MasterRelationRefreshGrpcAuthority Instance =>
            LazyInstance.Value;

        public ulong Publish(string worldGroup, long relationId)
        {
            if (string.IsNullOrWhiteSpace(worldGroup))
            {
                throw new ArgumentException(
                    "RelationRefresh requires a non-empty WorldGroup.",
                    nameof(worldGroup));
            }
                           ...
                           options.DeadlineMilliseconds)))
            {
                ...
                for (int attempt = 1; attempt <= MaximumAttempts; attempt++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            Logger.Info(
                                "[CALLBACK_RELATION_GRPC_ACCEPTED] WorldGroup=" +
                                worldGroup +
                                " RelationId=" +
                                relationId +
                                " Sequence=" +
                        ...
                        {
                            throw new InvalidOperationException(
                                "Authoritative RelationRefresh publication failed with " +
                                response.Result + ".");
                        }

                        lastException = new InvalidOperationException(
                            "Authoritative RelationRefresh publication returned " +
                            response.Result + ".");
                    }
                    catch (RpcException exception)
                        when (exception.StatusCode == StatusCode.Unavailable ||
                ...
                throw new InvalidOperationException(
                    "Authoritative RelationRefresh gRPC publication failed closed; no SCS callback was attempted.",
                    lastException);
            }
        }
    }
