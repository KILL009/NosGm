...
namespace NosGm.Master.Server
{
    internal class AuthentificationService : ScsService, IAuthentificationService
    {
        ...
        public bool Authenticate(string authKey)
        {
            ...
            if (!ServerConfiguration.EnableGameforgeTokenLogin || !HasSecureGameforgeKeys())
            {
                return false;
            }
        ...
        public bool RegisterGameforgeWorldPermit(long accountId, int sessionId, string ipAddress)
        {
            ...
            int configuredTtl = ServerConfiguration.GameforgeWorldPermitTtlSeconds;
            int ttlSeconds = Math.Max(15, Math.Min(600, configuredTtl));
            return GameforgeWorldPermitStore.Instance.TryIssue(
                accountId,
                sessionId,
                ipAddress,
                TimeSpan.FromSeconds(ttlSeconds));
        ...
        {
            return ServerConfiguration.EnableGameforgeTokenLogin &&
                   IsLegacyAuthClient() &&
