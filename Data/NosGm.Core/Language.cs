...
namespace NosGm.Core
{
    public sealed class Language
    {
        ...
        private static string NormalizeKnownCulture(string cultureName)
        {
            ...
            if (candidate == "uk" || candidate == "gb" || candidate == "english" || candidate.StartsWith("en-")) candidate = "en";
            else if (candidate == "spanish" || candidate == "español" || candidate == "espanol" || candidate.StartsWith("es-")) candidate = "es";
            else if (candidate == "german" || candidate == "deutsch" || candidate.StartsWith("de-")) candidate = "de";
            else if (candidate == "french" || candidate == "français" || candidate == "francais" || candidate.StartsWith("fr-")) candidate = "fr";
            else if (candidate == "italian" || candidate == "italiano" || candidate.StartsWith("it-")) candidate = "it";
            else if (candidate == "polish" || candidate == "polski" || candidate.StartsWith("pl-")) candidate = "pl";
            else if (candidate == "cz" || candidate == "czech" || candidate == "čeština" || candidate == "cestina" || candidate.StartsWith("cs-") || candidate.StartsWith("cz-")) candidate = "cs";
            else if (candidate == "russian" || candidate == "русский" || candidate.StartsWith("ru-")) candidate = "ru";
            else if (candidate == "jp" || candidate == "japanese" || candidate == "日本語" || candidate.StartsWith("ja-") || candidate.StartsWith("jp-")) candidate = "ja";
            else if (candidate == "cn" || candidate == "chinese" || candidate == "中文" || candidate.StartsWith("zh-") || candidate.StartsWith("cn-")) candidate = "zh";
    ...
    {
        public ClientLanguageProfile(byte regionType, int loginPort, string protocolPrefix, string clientFileSuffix, string serverCulture)
        {
    ...
    public static class ClientRegionMap
    {
        ...
        {
            if (!TryGetProfile(regionType, out ClientLanguageProfile profile)) throw new ArgumentOutOfRangeException(nameof(regionType));
        ...
        public static bool TryResolveLoginPort(int loginPort, out byte regionType, out string culture)
        {
            int regionIndex = loginPort - BaseLoginPort;
            if (regionIndex < 0 || regionIndex >= Profiles.Length || Profiles[regionIndex].LoginPort != loginPort)
            {
                regionType = 0;
                culture = null;
                return false;
        ...
        public static bool TryStripProtocolPrefix(string protocolUsername, out string accountName, out ClientLanguageProfile profile)
        {
            accountName = null;
            profile = null;
            if (string.IsNullOrWhiteSpace(protocolUsername)) return false;

            foreach (ClientLanguageProfile candidate in Profiles)
            {
                string prefix = candidate.ProtocolPrefix + "_";
                if (!protocolUsername.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)) continue;
        ...
        public static bool IsProtocolUsernameForAccount(string protocolUsername, string accountName, byte regionType)
        {
            if (string.IsNullOrEmpty(protocolUsername) || string.IsNullOrEmpty(accountName) || !TryGetProfile(regionType, out ClientLanguageProfile profile)) return false;
            return string.Equals(protocolUsername, profile.ProtocolPrefix + "_" + accountName, StringComparison.Ordinal);
        }
    }
}