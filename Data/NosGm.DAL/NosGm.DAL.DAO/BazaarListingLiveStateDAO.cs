...
{
    /// <summary>
    /// Commits a bazaar listing from the server-side live inventory snapshot.
    /// Online character memory is authoritative for mutable gold and item fields,
    /// while SQL is still used to reject identity, owner and duplicate conflicts.
    /// </summary>
    public sealed class BazaarListingLiveStateDAO
    {
        ...
        public BazaarListingResult Commit(BazaarListingDTO request)
        {
            ...
            try
            {
                using (var context = DataAccessHelper.CreateContext())
                {
                    ...
                    using (var transaction = context.Database.BeginTransaction(IsolationLevel.Serializable))
                    {
                        ...
                        if (seller == null || itemDefinition == null ||
                        ...
                        if (!IsTradableLiveSource(request.SourceBefore, itemDefinition))
                        {
                            transaction.Rollback();
                            return BazaarListingResult.InvalidItem;
                        }

                        if (!TryLoadOrMaterializeSource(context, request, out EfItemInstance source))
                        {
                            transaction.Rollback();
                            return BazaarListingResult.StateChanged;
                        }
                        ...
                        if (!ValidateInventoryPlan(request))
                        {
                            transaction.Rollback();
                            return BazaarListingResult.StateChanged;
                        }
                        ...
                        Guid sourceSerial = request.SourceBefore.EquipmentSerialId;
                        ...
                        // The online session owns the current mutable balance. Persist its
                        // calculated after-state instead of rejecting a harmless stale SQL row.
            ...
            {
                return ResolveDuplicateResult(request, exception);
            }
        ...
        private static bool IsRequestValid(BazaarListingDTO request)
        {
            return request != null &&
                   ...
                   request.SourceBefore.CharacterId == request.SellerCharacterId &&
                   request.SourceBefore.ItemVNum > 0 &&
                   request.SourceBefore.Amount > 0 &&
                   IsSupportedInventory(request.SourceBefore.Type) &&
        ...
        private static bool IsSupportedInventory(InventoryType type) =>
            type == InventoryType.Equipment ||
            type == InventoryType.Main ||
            type == InventoryType.Etc;

        private static bool IsTradableLiveSource(ItemInstanceDTO source, Item itemDefinition)
        {
            if (!IsSupportedInventory(source.Type))
            {
                return false;
            }
        ...
        private static bool TryLoadOrMaterializeSource(
            NosGmContext context,
            BazaarListingDTO request,
            out EfItemInstance source)
        {
            ItemInstanceDTO live = request.SourceBefore;
            source = context.ItemInstance.FirstOrDefault(item => item.Id == live.Id);

            if (context.BazaarItem.Any(listing => listing.ItemInstanceId == live.Id))
            {
                return false;
            }

            if (HasSerialConflict(context, live.Id, live.EquipmentSerialId))
            {
                return false;
            }
            ...
            if (source == null)
            {
                source = new EfItemInstance();
                ItemInstanceMapper.ToItemInstance(live, source);
                context.ItemInstance.Add(source);
                return true;
            }

            if (source.CharacterId != request.SellerCharacterId ||
                source.ItemVNum != live.ItemVNum ||
                source.Type == InventoryType.Bazaar)
            {
                return false;
            }

            Guid storedSerial = source.EquipmentSerialId ?? Guid.Empty;
            if (storedSerial != Guid.Empty &&
                live.EquipmentSerialId != Guid.Empty &&
                storedSerial != live.EquipmentSerialId)
            {
                return false;
            }
        ...
        private static bool HasSerialConflict(
            NosGmContext context,
            Guid itemInstanceId,
            Guid equipmentSerialId)
        {
            return equipmentSerialId != Guid.Empty &&
        ...
        private static bool ValidateInventoryPlan(BazaarListingDTO request)
        {
            ...
            if (before.Type == InventoryType.Equipment && !fullTransfer)
            {
                return false;
            }
            ...
            return sourceAfter != null &&
                   ...
                   bazaarAfter.Id != before.Id &&
                   bazaarAfter.EquipmentSerialId != Guid.Empty &&
                   bazaarAfter.EquipmentSerialId != before.EquipmentSerialId;
        }

        private static short? FindFreeBazaarSlot(NosGmContext context, long sellerCharacterId)
        {
...
SELECT @Result;";
        ...
        private static long LoadCompletedListingId(NosGmContext context, Guid operationId)
        {
...
WHERE OperationId = @OperationId;";
        ...
        private static BazaarListingResult ResolveDuplicateResult(
            BazaarListingDTO request,
            SqlException exception)
        {
            try
            {
                using (var context = DataAccessHelper.CreateContext())
                {
                    if (HasSchema(context))
                    {
                        long listingId = LoadCompletedListingId(context, request.OperationId);
                        if (listingId > 0)
                        {
                            request.Listing.BazaarItemId = listingId;
                            return BazaarListingResult.AlreadyCommitted;
                        }
                    }
                }
            ...
            {
                Logger.Error(
                    $"Unable to verify duplicate bazaar listing operation {request.OperationId}.",
                    lookupException);
            }

            Logger.Error(
                $"Bazaar listing identity conflict for operation {request.OperationId}.",
                exception);
            return BazaarListingResult.StateChanged;
        }

        private static void InsertOperation(NosGmContext context, BazaarListingDTO request, bool fullTransfer)
        {
            const string sql = @"
