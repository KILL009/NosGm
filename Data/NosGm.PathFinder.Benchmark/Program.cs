...
namespace NosGm.PathFinder.Benchmark
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("═══════════════════════════════════════════════════════════");
            Console.WriteLine();
            Console.WriteLine("  El principal problema de rendimiento del AStarPathFinder");
            Console.WriteLine("  actual es que aloca 3 matrices del TAMAÑO TOTAL del mapa");
            Console.WriteLine("  en CADA llamada (bool[,] closed, double[,] gScore,");
            Console.WriteLine("  GridPos[,] parent). Para un mapa de 150×150 eso son:");
            Console.WriteLine();
            Console.WriteLine($"    bool[150,150]    = {150 * 150 * 1:N0} bytes");
            Console.WriteLine($"    double[150,150]  = {150 * 150 * 8:N0} bytes");
            Console.WriteLine($"    GridPos[150,150] = {150 * 150 * 16:N0} bytes (aprox)");
            Console.WriteLine($"    TOTAL por llamada≈ {(150 * 150 * (1 + 8 + 16)) / 1024.0:F1} KB");
            Console.WriteLine();
            Console.WriteLine("  Con 500 monstruos moviendose al mismo tiempo = ~");
            Console.WriteLine($"    {500 * (150 * 150 * (1 + 8 + 16)) / 1024.0 / 1024.0:F1} MB alojados cada tick → GC Pressure alta.");
            Console.WriteLine();
            Console.WriteLine("  SOLUCIÓN RECOMENDADA: ThreadLocal<T> pool de matrices");
            Console.WriteLine("  reutilizables + versión número de generación para reset O(1).");
            Console.WriteLine("═══════════════════════════════════════════════════════════");
